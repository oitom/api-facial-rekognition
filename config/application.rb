Jets.application.configure do
  config.project_name = "api-facial-recognition"
  config.mode = "api"
  ...
  config.cors_preflight = {
    ...
    "access-control-allow-headers" => "Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,X-Amz-User-Agent,X-Idempotency-Key",
  }
  ...
  config.session.options = { secret: UUID.new.generate }
  config.hot_reload=false

  config.gems.disable = true
  config.lambda.layers = [
    ENV['CUSTOM_LAYER_ARN'].tr("\"", "")
  ] unless ENV['CUSTOM_LAYER_ARN'].nil?
end
